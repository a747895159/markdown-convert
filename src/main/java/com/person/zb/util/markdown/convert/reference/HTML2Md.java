...
import com.person.zb.util.markdown.convert.html2md.util.MDLine;
...
public class HTML2Md {
    ...
    public static MutablePair<String, String> parseURL2Md(String url) {
        try {
            MutablePair<String, String> pair = convert(new URL(url), "utf-8", true, false);
    ...
    public static MutablePair<String, String> convert(URL url, String charset, boolean needTitle, boolean returnHtml) {
        String title = null;
        String content = null;
        try {
            // 获取正文内容，
            ...
            // 获取标题
            if (needTitle) {
                title = fetchTitle(doc).replaceAll("-CSDN博客", "");
            }
            // 通用格式化
            HostEnums host = HostEnums.position(url);
            if (BooleanUtils.isTrue(host.getSyncFlag())) {
                // 编码格式化
                doc = Jsoup.parse(doc.html(), charset);
            } else {
                throw new RuntimeException("暂不支持以后再实现");
            }
            ...
            // 判断是否返回html,是就直接返回
            if (returnHtml) {
                return MutablePair.of(title, ele.html());
            }
        ...
        return MutablePair.of(title, content);
    }
    ...
    private static void handlerWebSite(HostEnums position, Element content) {
        switch (position) {
            case CSDN:
                ...
                break;
            case CNBLOG:
                handleCnblog(content);
                break;
            case WECHAT:
                handleWechat(content);
                break;
            default:
        }
    }
